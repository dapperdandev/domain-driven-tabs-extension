...
const getTitleFromDomain = (domain: string): string => {
  ...
  const result = domainArr
    .slice(0, domainArr.length - 1)
    .join('.')
    .replace('www.', '');
...
const getTabGroupMap = async (): Promise<Record<string, number[]>> => {
  const tabs = await chrome.tabs.query({ currentWindow: true });
  return tabs.reduce(
    (prev: Record<string, number[]>, curr: Tab): Record<string, number[]> => {
...
export const getDomainMap = async (): Promise<Record<string, number[]>> => {
  const tabs = await chrome.tabs.query({ currentWindow: true });
  const domainMap = tabs.reduce(
    (prev: Record<string, number[]>, curr: Tab): Record<string, number[]> => {
      if (!curr.url) return prev;
      const domain = new URL(curr?.url).hostname;
      return {
        ...prev,
        [domain]: [...(prev[domain] || []), curr.id as number],
      };
    },
    {}
  );
...
export const groupTabs = async (
  ...
  domains.forEach((domain: string): void => {
    ...
    chrome.tabs.group({ tabIds: domainMap[domain] }, (groupId: number) =>
      promises.push(chrome.tabGroups.update(groupId, options))
    );
  });
